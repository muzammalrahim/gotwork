@isset($news_feed->properties['old']['bid_amount'])
            ...
            <div class="lg:w-11/12 md:w-12/12 sm:w-12/12 w-12/12">
                {{-- @if($news_feed->description == "Bid updated") --}}
                    ...
                    <p class="text-sm ml-4 w-5/6">
                        <a class="text-blue-500"> {{(Auth::user()->id == $news_feed->causer_id)? 'You ': $causer_data->name}} </a> have updated bid from {{$bid_data->project_currency_symbol}} {{ $news_feed->properties['old']['bid_amount'] }} to {{$bid_data->project_currency_symbol}} {{ $news_feed->properties['attributes']['bid_amount'] }}
                        on <a class="text-blue-500"> {{$project_data->title}} </a>
                    </p>
